/*
Host: ecoon1.lmu.build
Username: ecoonlmu_dba
Password: sql_2022
*/
...
VALUES ('Ooba', 'Man kills monster', 2003, 2, 2, 4, 3, 120, 10, 'R', 'special features');
...
VALUES ("Christoph", "Waltz"), ("Dwayne", "Johnson");
/*
...
*/
INSERT INTO users (first_name, last_name, email, zip_code)
VALUES ('Ethan', 'Coon', 'ethandcoon@gmail.com', 90045);

/*
18. INSERT 1 work experience entry for the user just inserted. (2 points)
*/
INSERT INTO work_experience (user_id, company_id, years_worked, position_title)
VALUES (1, 1, 1, 'Door Greeter')
...
*/
SELECT ContactName, CompanyName, Address, City, Region, PostalCode, Country
FROM Customers
...
FROM Products
WHERE QuantityPerUnit LIKE "%glass%" OR QuantityPerUnit LIKE "%jar%" OR QuantityPerUnit LIKE "%bottle%"
AND Discontinued = 0;
...
*/
SELECT FirstName, LastName, FLOOR(DATEDIFF(NOW(), BirthDate)/365) AS Age
FROM Employees
ORDER BY Age DESC
LIMIT 1;
/*
...
*/
SELECT OrderID,
	(SELECT SUM(UnitPrice * Quantity * (1-Discount)) FROM OrderDetails od2 WHERE od.OrderID = od2.OrderID) AS OrderTotal,
	ProductID, Quantity,
	UnitPrice * Quantity AS BeforeDiscount,
	UnitPrice * (1-Discount) * Quantity AS AfterDiscount
...
*/
SELECT SUM(UnitsInStock * UnitPrice) AS TotalInStock
FROM Products;
...
VALUES("Coon", "Ethan", "Chief Operating Officer", "Mr.", '2000-07-23', '2022-02-02', "8655 Belford Ave", "Los Angeles", "CA", 90045, "USA", "(408) 674-7097", 2211, "Ooga booga, lichuga!", 3, "ethancoon.com");
...
SELECT e.EmployeeID, e.FirstName, e.LastName, od.ProductID, SUM(od.Quantity) AS NumSold
FROM Employees e 
JOIN Orders o
ON o.EmployeeID = e.EmployeeID
JOIN OrderDetails od
ON o.OrderID = od.OrderID
GROUP BY e.EmployeeID, ProductID
HAVING NumSold >= 150
ORDER BY NumSold DESC;
-- how do I limit this to only 1 result per employeeID?
...
*/
SELECT c.CustomerID, c.CompanyName, ROUND(SUM(od.UnitPrice * Quantity * (1-Discount)),2) AS TotalSpent
FROM Customers c
JOIN Orders o
ON c.CustomerID = o.CustomerID 
JOIN OrderDetails od 
...
*/
SELECT EmployeeID, COUNT(ShippedDate > RequiredDate) AS TimesLate
FROM Orders
GROUP BY EmployeeID
ORDER BY TimesLate DESC;

-- Custom Data Requests

/*
Data Request 1
Question: Which shipper do we use the most?

Business Justification: Knowing which shipper we use most is important for building a good, long lasting relationship with the company.

*/

-- SQL 1
SELECT s.ShipperID, s.CompanyName, COUNT(o.OrderID) AS TimesShipped
FROM Shippers s
JOIN Orders o
ON s.ShipperID = o.ShipVia
GROUP BY s.ShipperID
ORDER BY TimesShipped DESC;

-- SQL 2: What companies do we ship to the most, and where are they located?
SELECT o.ShipName, COUNT(o.OrderID) AS TimesShipped, o.ShipCountry, s.CompanyName as ShippedBy
FROM Orders o
JOIN Shippers s 
...
-- SQL 3: How much cargo in weight does each shipper deliver per order, on average?
SELECT ShipperID, CompanyName, ROUND(SUM(Freight)/COUNT(OrderID),2) AS AverageWeight
FROM Orders o
JOIN Shippers s
ON o.ShipVia = s.ShipperID 
GROUP BY ShipVia
...
/*
Data Request 2
Question: Which products net the highest revenue?

Business Justification: We can focus on the sales of these products which create the most revenue.
*/ 
...
-- SQL 2: What categories of products net the highest revenue?
SELECT p.CategoryID, c.CategoryName, SUM(od.UnitPrice * od.Quantity * (1 - od.Discount)) AS TotalRevenue
FROM OrderDetails od
JOIN Products p
ON od.ProductID = p.ProductID 
JOIN Categories c 
...
-- SQL 3: Which products within the beverages category generate the most revenue?
